...
export function DiscoveryTab({ state, setState }: DiscoveryTabProps) {
  ...
  const setNote = (id: string, value: string) => {
    setState((s) => ({ ...s, discoveryNotes: { ...s.discoveryNotes, [id]: value } }));
  };
  ...
  return (
    <div className="space-y-8">
      {/* Client Snapshot */}
      <SectionCard title="Client Snapshot" subtitle="Basic client information for this session">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <Field label="Full Name">
            <Input value={state.clientName} onChange={(e) => setForm("clientName", e.target.value)} placeholder="Jane Smith" />
          </Field>
          <Field label="Location">
            <Input value={state.clientLocation} onChange={(e) => setForm("clientLocation", e.target.value)} placeholder="Brisbane, QLD" />
          </Field>
          <Field label="Occupation">
            <Input value={state.clientOccupation} onChange={(e) => setForm("clientOccupation", e.target.value)} placeholder="Engineer" />
          </Field>
          <Field label="Client Type">
            <Select value={state.clientType} onChange={(e) => setForm("clientType", e.target.value)} options={clientTypeOptions} placeholder="Select type..." />
          </Field>
          <Field label="Income Range">
            <Input value={state.clientIncomeRange} onChange={(e) => setForm("clientIncomeRange", e.target.value)} placeholder="$100k – $150k" />
          </Field>
          <Field label="Preferred Locations">
            <Input value={state.clientPreferredLocations} onChange={(e) => setForm("clientPreferredLocations", e.target.value)} placeholder="Brisbane, Gold Coast" />
          </Field>
        </div>
      </SectionCard>
      ...
      <div className="flex gap-6">
        {/* Sidebar */}
        <div className="hidden md:flex flex-col gap-1.5 w-64 flex-shrink-0">
          <p className="text-base font-bold text-zinc-400 px-2 mb-2">Call Sections</p>
          {discoverySections.map((s, i) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 text-left px-4 py-3 rounded-xl text-lg font-medium transition-all ${
                  isActive
                    ? "bg-yellow-600/20 text-yellow-400 border border-yellow-600/40"
                    : "text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800"
                }`}
              >
                <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2 flex-shrink-0 ${
                  hasNote ? "bg-green-600/20 border-green-500 text-green-400"
                  : isActive ? "border-yellow-500 text-yellow-400"
                  : "border-zinc-600 text-zinc-500"
                }`}>
                  {hasNote ? <CheckCircle2 size={14} /> : i + 1}
                </span>
        ...
        <div className="flex-1 min-w-0">
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 overflow-hidden">
            {/* Section Header */}
            <div className="px-8 py-6 border-b border-zinc-800 bg-zinc-800/50">
              <span className="text-base font-bold text-yellow-500">
                Step {activeSectionIdx + 1} of {discoverySections.length}
              </span>
              <h3 className="text-2xl font-bold text-zinc-100 mt-1">{section.title}</h3>
              <p className="text-base text-zinc-400 mt-1 leading-relaxed">{section.purpose}</p>
            </div>

            <div className="p-8 space-y-8">
              {/* Script */}
              {section.script && (
                <div className="bg-zinc-800/60 border border-yellow-600/20 rounded-2xl p-6">
                  <div className="flex items-center gap-3 mb-4">
                    <MessageSquare size={20} className="text-yellow-500" />
                    <span className="text-base font-bold text-yellow-500 uppercase tracking-wide">Suggested Script</span>
                  </div>
                  <p className="text-lg text-zinc-200 leading-relaxed italic">{section.script}</p>
                </div>
              )}
              ...
              {section.questions && section.questions.length > 0 && (
                <div>
                  <p className="text-base font-bold text-zinc-400 uppercase tracking-wide mb-4">Questions to Ask</p>
                  <ul className="space-y-4">
                    {section.questions.map((q, i) => (
                      <li key={i} className="flex items-start gap-3 text-lg text-zinc-200 leading-relaxed">
                        <span className="text-yellow-500 flex-shrink-0 text-2xl leading-tight">›</span>
                        {q}
                      </li>
                    ))}
                  </ul>
              ...
              {section.listenFor && section.listenFor.length > 0 && (
                <div className="bg-blue-950/30 border border-blue-800/30 rounded-2xl p-6">
                  <p className="text-base font-bold text-blue-400 uppercase tracking-wide mb-4">Listen For</p>
                  <div className="flex flex-wrap gap-3">
                    {section.listenFor.map((item, i) => (
                      <span key={i} className="text-base bg-blue-900/30 border border-blue-700/30 text-blue-300 rounded-full px-4 py-2">
                        {item}
                      </span>
                    ))}
                  </div>
              ...
              <div>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <p className="text-base font-bold text-zinc-400">Consultant Notes</p>
                </div>
                <Textarea
              ...
              <div className="flex items-center justify-between pt-2">
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-3 text-base font-semibold text-zinc-300 hover:text-zinc-100 disabled:opacity-30 disabled:cursor-not-allowed transition-colors border border-zinc-700 hover:border-zinc-500 rounded-xl"
                >
                ...
                <div className="flex gap-2 md:hidden">
                  {discoverySections.map((_, i) => (
                    <button key={i} onClick={() => setActiveSectionIdx(i)}
                      className={`w-3 h-3 rounded-full transition-colors ${i === activeSectionIdx ? "bg-yellow-500" : "bg-zinc-700"}`}
                    />
                  ))}
                </div>

                <button
                  onClick={() => setActiveSectionIdx((i) => Math.min(discoverySections.length - 1, i + 1))}
                  disabled={activeSectionIdx === discoverySections.length - 1}
                  className="flex items-center gap-2 px-6 py-3 text-base font-bold text-zinc-900 bg-yellow-600 hover:bg-yellow-500 disabled:opacity-30 disabled:cursor-not-allowed transition-colors rounded-xl"
                >
...
function QDivider({ label }: { label: string }) {
  return (
    <div className="flex items-center gap-4 pt-4">
      ...
      <span className="text-base text-zinc-400 font-bold">{label}</span>
      <div className="h-px flex-1 bg-zinc-800" />
    </div>
  );
}
...
function OwnerOccupierForm({ state, setState }: DiscoveryTabProps) {
  ...
  const set = (key: string, val: string) =>
    setState((s) => ({ ...s, ownerOccupier: { ...s.ownerOccupier, [key]: val } }));

  return (
    <div className="space-y-6">
      <QDivider label="Owner Occupier Questionnaire" />
      ...
      <SectionCard title="Current Living Situation" collapsible defaultOpen>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <Field label="What They Like"><Textarea value={f.likesAboutCurrent} onChange={(e) => set("likesAboutCurrent", e.target.value)} placeholder="Close to work, good schools..." /></Field>
          <Field label="What They Dislike"><Textarea value={f.dislikesAboutCurrent} onChange={(e) => set("dislikesAboutCurrent", e.target.value)} placeholder="Too small, no yard..." /></Field>
        </div>
      </SectionCard>
      ...
      <SectionCard title="Finance" collapsible defaultOpen>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <Field label="Other Liabilities" className="sm:col-span-2"><Input value={f.liabilities} onChange={(e) => set("liabilities", e.target.value)} placeholder="Car loan $15k, HECS $30k" /></Field>
        </div>
      </SectionCard>
      ...
      <SectionCard title="Lifestyle & Decision Making" collapsible>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <Field label="Future Plans"><Textarea value={f.futurePlans} onChange={(e) => set("futurePlans", e.target.value)} placeholder="May expand family..." /></Field>
          ...
          <Field label="Key Blockers" className="sm:col-span-2"><Textarea value={f.blockers} onChange={(e) => set("blockers", e.target.value)} placeholder="Not sure what suburb is best value..." /></Field>
        </div>
      </SectionCard>
    </div>
  );
...
function InvestorForm({ state, setState }: DiscoveryTabProps) {
  ...
  const set = (key: string, val: string) =>
    setState((s) => ({ ...s, investor: { ...s.investor, [key]: val } }));

  return (
    <div className="space-y-6">
      <QDivider label="Investor Questionnaire" />
